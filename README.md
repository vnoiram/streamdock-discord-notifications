...
It listens on `http://127.0.0.1:41921/` for WebSocket upgrades, requests Windows notification listener access, and filters toast notifications whose app display name contains the configured app name. History is memory-only by default. When `Persist` is enabled, history is saved under `%LOCALAPPDATA%\StreamDock\discord-notifications-history.json` unless `Save path` overrides it. Saved history is encrypted by default with Windows DPAPI, so the same Windows user account is required to read it. The Stream Dock Property Inspector can send a runtime `configure` command to change persistence, encryption, the history file, and the maximum retained items.
